...
from utils import (
    rotate_in_x,
    rotate_in_z,
    get_corrected_lab_coordinates_in_reciprocal_units,
)
import settings
...
for line in stream:
    if reading_chunks:
        ...
        if line.startswith("End of peak list"):
            ...
            PF8Config.pixel_maps["phi"] = np.arctan2(
                PF8Config.pixel_maps["y"], PF8Config.pixel_maps["x"]
            )

        elif reading_peaks:
            fs, ss, dump, intensity = [float(i) for i in line.split()[:4]]
            if intensity > 0:
                x_lab, y_lab, z_lab = get_corrected_lab_coordinates_in_reciprocal_units(
                    int(fs), int(ss), PF8Config.pixel_maps, k, res, clen
                )
                x, y, z = rotate_in_z(x_lab, y_lab, z_lab, azimuth_of_the_tilt_axis)
                x, y, z = rotate_in_x(
                    x, y, z, starting_angle + (event_number * tilt_angle)
                )
                x, y, z = rotate_in_z(x, y, z, -1 * azimuth_of_the_tilt_axis)
                reciprocal_space[
                    int(reciprocal_space_radius + space_stretching_factor * z),
                    int(reciprocal_space_radius + space_stretching_factor * y),
                    int(reciprocal_space_radius + space_stretching_factor * x),
                ] += (
        ...
        PF8Config.set_geometry_from_file()
        reciprocal_space_radius = (
            int((max(PF8Config.get_detector_center()) * k) / (res * clen))
            * space_stretching_factor
        )
        reciprocal_space_dimension = 2 * reciprocal_space_radius
        reciprocal_space = np.zeros(
            (
                reciprocal_space_dimension + 1,
                reciprocal_space_dimension + 1,
                reciprocal_space_dimension + 1,
            ),
            dtype=np.int32,
...
tif.imwrite(
    info["output_folder"] + "/" + splitext(basename(sys.argv[1]))[0] + "-merge.tif",
    reciprocal_space,
)
